...
\paragraph{Chrosen plaintext}In questo caso l'attaccante può scegliere \underline{arbitrariamente} i testi da cifrare. Anche questa situazione non è improbabile: nella crittografia asimmetrica, la chiave pubblica è nota a tutti e può essere utilizzata per cifrare quanti messaggi si desidera.
...
\subsection{Crittoanalisi} Uno schema di crittografia e considerato sicuro dal punto di vista computazionale se il testo cifrato generato dallo schema soddisfa uno o entrambi i seguenti criteri:
\begin{itemize}
...
\begin{itemize}
	...
	\bitem{facilità di analisi}se l'algoritmo puù essere spiegato in modo coinciso e chiaro, è più facile analizzarlo per vulnerabilità crittoanalitiche e quindi sviluppare un livello più elevato di garanzia sulla sua robustezza.
\end{itemize}
...
\begin{enumerate}
	...
	\item si calcola il loro prodotto $n=pq$, chiamato \textit{modulo} (dato che tuta l'aritmetica seguente è modulo $n$), e il prodotto $\varphi(n) = (p-1(q-1))$, dove $\varphi(n)$ è la funzione toziente;
