...
\end{align*}
 ...
 Dati $m \ge 1$ e $a, b$ interi, allora
\begin{align*}
    a \cdot b = 1 \pmod m
\end{align*}
 ...
 \underline{Esempio}: dati $m = 5$ e $a = 2$, $GDC(5, 2) = 1$ e quindi esiste un inverso di $2 \pmod 5$. L'inverso è $3$, in quanto $2\cdot 3 \equiv 1 \pmod 5$.
...
\end{align*}
 ...
 ovvero i primi \textit{n} numeri naturali a partire dallo $0$. La rappresentazione canonica in un'algebra a modulo n è un numero compreso tra $0$ e $n-1$.
\begin{figure}[thbp]
	\centering
	\fbox{%
		\parbox{.9\textwidth}{%
...
\end{itemize}
 ...
 Una ``cosa'' che ha le proprietà di associatività, elemento neutro e elemento inverso è detta \textbf{gruppo}, mentre una che gode di tutte e quattro le proprietà è detta \textbf{gruppo abeliano}. Quindi l'insieme $Z_n$ con l'operazione di somma è un gruppo.
...
\end{align*}
 ...
 Quindi la classe a cui appartiene $x$ è l'inverso di $a$. Ho confermato che $Z^*_n$ con il prodotto è un gruppo.
...
\end{theorem}
 ...
 Per questo teorema, se prendo un elemento qualsiasi di un gruppo e lo elevo alla cardinalità del gruppo, otteno $1$. Quindi in questa catena l'elemento neutro c'è, che sarebbe $g^0 = 1$.
...
\begin{theorem}[Teorema di Lagrange]
    Se $G'$ è un sottogruppo di $G$, allora $|G'|$ divide $|G|$, ovvero il numero di elementi di $G'$ è divisore del numero di elementi di $G$.
\end{theorem}

 \underline{Esempio}\\

 Provo ora a costruire tutti i sottogruppi di $Z^*_{15} = \{1, 2, 4, 7, 8, 11, 13, 14\}$:
...
\end{align*}
 ...
 Ho confermato che la cardinalità di tutti i generatori divide la cardinalità dell'insieme originale, ma nessuno di questi generatori è ciclico. Esistono generatori ciclici? Si, almeno quando $n$ è primo. In generale dato $p$ numero primo, il rispettivo insieme sarà $Z^*_{p} = \{1, ..., p-1\}$.
...
\end{theorem}
 ...
 Provo ora a cercare un generatore di $Z^*_{7} = \{1, 2, 3, 4, 5, 6\}$:
\begin{align*}
...
\end{theorem}
 ...
 Vediamo la liste delle potenze dei numeri $1, 2, ..., 6$ in modulo con il numero primo $7$:
...
\begin{align*}
 	g^{2i}=
 	    \begin{cases}
 	   g^i \\ 
 	    g^{i+\frac{p-1}{2}} &\xrightarrow[\text{quadrato}]{\text{elevamento al}} \left(g^{i+\frac{p-1}{2}}\right)^2 = g^{2i}\cdot g^{p-1} = g^{2i}\cdot 1
        \end{cases} 
\end{align*}
...
\paragraph{Quadrati in $Z_{pq}^*$} Dato $Z_{pq}^*$, con $p, q$ numeri primi, il numero di quadrati contenuto nell'insieme è $\frac{\varphi}{4}$, ovvero il numero di quadrati comuni a $Z_{p}^*$ e $Z_{q}^*$.
 Dato $p$ numero primo, come è possibile capire se un dato numero $a$ sia uguale a un quadrato modulo $p$? Escludendo il risolvere il problema del logaritmo discreto per $a$ e verificare se il risultato è pari o dispari, un modo per farlo è verificare se $x_i^2 = a$, per ogni $x_i \in 0, 1, 2, 3, 4, ..., p-1$. Esistono modi più efficienti? 
...
Il simbolo di Legendre ritorna $1$ se $a$ è un quadrato modulo $p$ o $-1$ se non lo è.  Infatti:
\begin{itemize}
...
\end{align*}
 ...
 allora il simbolo di Jacobi $\binom{a}{n}$ è definito dalla formula 
\begin{align*}
    \binom{a}{n} = \binom{a}{p_1}^{e_1} \binom{a}{p_2}^{e_2} \dots\ \binom{a}{p_t}^{e_t}
\end{align*}
\end{definition}

 Il simbolo di Jacobi $\binom{a}{n}$ gode delle seguenti proprietà:
\begin{align*}
    \binom{a}{n_1n_2} = \binom{a}{n_1}\binom{a}{n_2}
\end{align*}
\begin{align*}
        \binom{a_1a_2}{n} = \binom{a_1}{n}\binom{a_2}{n}
\end{align*}

 Se $n$ è un numero primo, allora il simbolo di Jacobi equivale al simbolo di Legendre. Se scompongo $n$ in numeri primi, allora ogni simbolo di Jacobi $\binom{a}{p_i}$ mi ritorna $1$ o $-1$:
\begin{align*}
    \binom{a}{n}&= \binom{a}{pq}= \binom{a}{p}\binom{a}{q}
\end{align*}
...
Tornando al caso iniziale $n = pq$, nel calcolo del simbolo di Jacobi $\binom{a}{pq}$ si possono incontrare tre casi:
\begin{enumerate}
    \item $a$ è un quadrato sia in $Z_p^*$ che in $Z_q^*$ (probabilità $\frac{1}{4}$):
        \begin{align*}
            \binom{a}{pq} = \binom{a}{p}\binom{a}{q} = 1 \cdot 1 = 1
        \end{align*}
...
\end{enumerate}
 ...
 Dai tre casi sopra, si può capire che il simbolo di Jacobi è limitato rispetto al simbolo di Legendre, in quanto è in grado solamente di decidere se un numero è un non quadrato nel caso particolare in cui è un quadrato in $Z_p^*$ e non quadrato in $Z_q^*$ (o viceversa).
...
\end{align*}
 ...
 Questo metodo è chiamato \textbf{iterative squaring} e permette di calcolare le potenze in un numero di moltiplicazioni proporzionale ai bit di $b$. Calcola via via le potenze di $a$, elevate a potenze di 2, e verifica a vedere se il corrispondente bit dell'espressione binaria di $b$ vale $0$ o $1$: se vale $0$ non fa nulla, se vale $1$ moltiplica il risultato parziale per la potenze che ha calcolato.
...
\end{itemize}
 ...
 Dato $a \in Z_n^*$ con $\left(\frac{a}{n}\right) = 1$, vogliamo ottenere un oggetto con la stessa quadraticità di $a$, ma distribuito uniformemente tra gli oggetti con la stessa quadraticità. Si può fare così:
\begin{align*}
...
\begin{align*}
    ...
    &(x+y)(x-y) = kn
\end{align*}
\footnote{il resto della divisione per $n$ è 0, quindi il prodotto è un multiplo di $n$}
...
\end{proof}
 ...
 Supponiamo che esista un algoritmo $A \in PPT$ (Probabilistic Polinomial Time) in grado di calcolare la radice quadrata di un numero:
