...
\begin{itemize}
    ...
    \item $B$ non deve essere in grado di indovinare $b$\footnote{Se noi costruiamo un algoritmo dove: Alice sceglie il bit $b$, genera il messaggio $m$, lo invia a Bob e Bob applica un qualche algoritmo per produrre un nuovo bit, la probabilità che questo sia uguale a $b$ differisce da $\frac{1}{2}$ di una quantità più piccola di ogni polinomio}.
\end{itemize}
...
\end{align*}
 cioè dovrebbe trovare due chiavi tali per cui una permetta di aprire la busto rivelando il bit $0$, mentre la seconda permetta di aprire la busta rivelando il bit $1$ (su AES non siamo in grado di farlo).
\\
...
\begin{itemize}
    ...
    \item $A$ per creare la busta chiusa necessita del supporto di $B$, che deve generare $R$\footnote{senza $R$ il messaggio diventerebbe arbitrario - contiene solo il bit scelto da $A$ - e quindi creare buste con valori diversi sarebbe banale, in quanto metà delle chiavi decodificherebbero un cyphertext come $0$ e l'altra $1$}. Se $B$ non invia $R$, $A$ non può inviare il bit.
\end{itemize}
...
\end{itemize}
 ...
 Se tutti gli $r_i$ dovessero valere $0$, allora in nessuno degli $y_i$ sarebbe presenta l'informazione riguardo il bit $b$. Ma la probabilità che questo avvenga è $\frac{1}{2^l}$, trascurabile. 
\\

 $A$ non può creare due buste in dovrebbe trovare due semi $s_0$ e $s_1$, tali per cui il primo porta alla stringa $x_1 \ ... \ x_l$ e il secondo alla stringa $x_1' \ ... \ x_l'$ e :
\begin{align*}
...
\begin{align*}
    s_n = s \oplus s_1 + ... \oplus s_{n-1}
\end{align*}

 Se $s_1 \ ... \ s_{n-1}$ sono sequenze casuali allora anche il loro XOR è casuale. Lo XOR tra $s$ e $s_1 \oplus ... \oplus s_{n-1}$ è anch'esso casuale. Per come abbiamo costruito $s_n$, vale che:
\begin{align*}
    s_1 + ... \oplus s_{n} = s
\end{align*}

 Abbiamo trovato un modo per per ripartire un segreto in $n$ parti tale per cui se mettiamo assieme le $n$ parti otteniamo il segreto, mentre se ne mettiamo assieme meno di $n$ parti non otteniamo il segreto.
\\
...
\end{align*}
 ...
 Per costruire un polinomio casuale di grado $k$ basta scegliere $a_1 \ ... \ a_{k-1}$ a caso (ponendo $a_0 = s$). Ad ogni agente $i$ comunico $P[i]$. Se $k$ agenti mettono insieme il loro segreto, conosciamo il valore del polinomio in $k$ punti.
...
\\
 ...
 Vediamo un protocollo che i crittografi possono seguire (\ref{fig:12-3}).
\begin{figure}
    \centering
